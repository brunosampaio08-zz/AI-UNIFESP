...
if __name__ == "__main__":
    ...
    clt_num = int(input("Input k for single linkage:"))

    sl_model = cluster.AgglomerativeClustering(n_clusters=clt_num)

    sl_model.fit(datalist)

    plt.scatter([k[0] for k in datalist], [k[1] for k in datalist], c = sl_model.labels_.astype(float))

    plt.savefig("IMAGES/"+filename+f"_sing-link_k{clt_num}.png")
